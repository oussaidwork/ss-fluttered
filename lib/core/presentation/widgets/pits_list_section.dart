...
class PitsListSection extends StatefulWidget {
  const PitsListSection({super.key});

  @override
  State<PitsListSection> createState() => _PitsListSectionState();
}

class _PitsListSectionState extends State<PitsListSection> {
  void _showPitDialog({Map<String, dynamic>? pit, String? docId}) {
    final nameCtrl =
        TextEditingController(text: pit?['name'] ?? '');
    final capacityCtrl = TextEditingController(
        text: pit?['capacity']?.toString() ?? '');
    final volumeCtrl = TextEditingController(
        text: pit?['currentVolume']?.toString() ?? '');
